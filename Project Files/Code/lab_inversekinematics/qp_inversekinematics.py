...
try:
    ...
    print(f"CRITICAL: IK Model Import Error: {e}")
...
class LabGUIExerciseDirect(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, e):
        for leg in self.emio.legs:
            ...
            elif hasattr(leg.leg, "BeamInterpolation"):
...
class LabGUIExerciseIK(Sofa.Core.Controller):
    def __init__(self, root, emio):
        ...
        MyGui.MoveWindow.setActuators([m.JointActuator.angle for m in emio.motors],
                                      [i for i in range(len(emio.motors))], "displacement")
...
def createScene(rootnode):
    ...
    # --- Enhanced Model Handling with Proxy ---
    requested_model = getListFromArgs(args.legsModel)[0]
    vol_variants = ["tetra_linear", "hyper", "nonuniform"]
    
    # Use "tetra" as a proxy for Emio's internal validation
    proxy_model = "tetra" if requested_model in vol_variants else requested_model
    
    # Apply factor for all Volume variants
    is_vol = (proxy_model == "tetra")
    legs_ym = [parameters.youngModulus * parameters.tetraYMFactor] if is_vol else [parameters.youngModulus]
    ...
    emio = Emio(name="Emio",
                ...
                legsYoungModulus=legs_ym,
                centerPartName=args.centerPartName,
                centerPartType="rigid",
                extended=True)
    
    # Inject grafted physics manually into the proxy legs
    for i, leg in enumerate(emio.legs):
        # Fix the IndexError by checking list length
        if i < len(args.legsName):
            args.legName = args.legsName[i]
        else:
            # Fallback to the first name provided if the list is too short
            args.legName = args.legsName[0]

        if requested_model == "tetra_linear" and tetra_linear:
            tetra_linear.create_model(leg, args, parameters, [0,0,0])
        elif requested_model == "hyper" and hyper:
            hyper.create_model(leg, args, parameters, [0,0,0])
        elif requested_model == "nonuniform" and nonuniform:
            nonuniform.create_model(leg, args, parameters, [0,0,0])
    ...
    if exercise1:
        ...
        if args.connection:
            rootnode.addObject(LabGUICamera(markers, rootnode))
